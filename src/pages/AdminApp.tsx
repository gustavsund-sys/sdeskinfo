...
import { useDisplayData } from "../hooks/useFirestoreData";
import {
  activeAt,
  localInput,
  swedishTime,
...
function Login() {
  ...
  async function submit(e: FormEvent) {
    e.preventDefault();
    if (!adminEmail) {
      setError("Inloggningen behöver konfigureras. Se README.");
      return;
    }
    setBusy(true);
    setError("");
  ...
  return (
    <main className="login-page">
      <section className="login-card">
        <div className="brand">
          <span className="brand-mark">ÖU</span>
          <span>Service desk</span>
        </div>
        <p className="kicker">Lösenordsskyddad administration</p>
        <h1>Logga in</h1>
        <form onSubmit={submit}>
          ...
          <button className="button primary" disabled={busy || !adminEmail}>
  ...
  return (
    <div className="admin-shell">
      <aside className="admin-nav">
        ...
        <button className="nav-signout" onClick={() => signOut(auth)}>
          <LogOut />
          Logga ut
        </button>
      </aside>
      <div className="admin-main">
        <Routes>
          ...
          <Route path="new" element={<MessageEditor />} />
          <Route path="edit/:id" element={<MessageEditor />} />
...
export function AdminApp() {
  ...
  if (user === undefined)
    return <div className="admin-loading">Service desk</div>;
...
function useHeartbeatStatus() {
  ...
  useEffect(
    () =>
      onSnapshot(doc(db, "displayStatus", "main"), (s) =>
        setLastSeen(toDate(s.data()?.lastSeen)),
      ),
    [],
  );
  return {
    lastSeen,
    online: !!lastSeen && Date.now() - lastSeen.getTime() < 120000,
  };
}
function Dashboard() {
  const { messages } = useDisplayData();
  const { lastSeen, online } = useHeartbeatStatus();
  const [now, setNow] = useState(new Date());
  useEffect(() => {
    const id = setInterval(() => setNow(new Date()), 15000);
    return () => clearInterval(id);
  }, []);
  const active = messages.filter((m) => activeAt(m, now));
  const upcoming = messages
    .filter((m) => upcomingAt(m, now))
    .sort(
  ...
  return (
    <>
      <header className="page-header">
        <div>
          <p className="kicker">Informationsskärm</p>
          <h1>Översikt</h1>
        </div>
      ...
      <section className="status-card">
        <div className={`status-icon ${online ? "online" : "offline"}`}>
          <Monitor />
        </div>
        <div>
          <p className="card-label">Skärmens status</p>
          <h2>{online ? "Online" : "Offline"}</h2>
          <p>Senast kontakt: {swedishTime(lastSeen)}</p>
        </div>
        <span className={`status-pill ${online ? "online" : "offline"}`}>
          {online ? "Kontakt" : "Ingen kontakt"}
        </span>
      </section>
      <MessageSection
        title="Just nu"
        empty="Ingen information visas just nu."
        messages={active}
      />
      <MessageSection
        title="Kommande"
        empty="Ingen information är schemalagd."
        messages={upcoming}
      />
    </>
  );
    ...
    <section className="admin-section">
      ...
      {!messages.length ? (
        ...
        <div className="message-list">
          {messages.map((m) => (
            <article
              className={`message-card priority-${m.priority}`}
              key={m.id}
            >
              <div>
                <div className="message-meta">
                  ...
                  <span>
                    <Clock3 /> {swedishTime(toDate(m.startAt))}
                  </span>
                </div>
                <h3>{m.title}</h3>
                <p>{m.body}</p>
              ...
              <div className="card-actions">
                <Link className="button secondary" to={`/admin/edit/${m.id}`}>
                  Redigera
                </Link>
                {activeAt(m) && (
                  <button
                ...
                <button
                  ...
                  aria-label={`Ta bort ${m.title}`}
                  onClick={() =>
                    confirm(`Ta bort ”${m.title}”?`) &&
...
function MessageEditor() {
  ...
  useEffect(() => {
    ...
    return onSnapshot(collection(db, "messageTemplates"), (snap) =>
      setTemplates(
        snap.docs.map(
          (item) => ({ id: item.id, ...item.data() }) as InfoTemplate,
        ),
      ),
    );
  }, [editing]);
  useEffect(() => {
    if (!editing) return;
    getDoc(doc(db, "messages", id)).then((s) => {
      if (s.exists()) {
        const m = s.data() as ServiceMessage;
        setDraft({
          title: m.title,
          body: m.body,
          type: m.type,
          priority: m.priority,
          start: localInput(toDate(m.startAt)!),
          end: toDate(m.endAt) ? localInput(toDate(m.endAt)!) : "",
        });
      }
    });
  }, [editing, id]);
  const previewMessage: ServiceMessage = {
    id: "preview",
    title: draft.title || "Rubrik",
    body: draft.body || "Skriv ett meddelande för att se förhandsvisningen.",
    type: draft.type,
  ...
  async function saveTemplate() {
    if (!draft.title.trim() || !draft.body.trim()) {
      setTemplateStatus("Fyll i rubrik och meddelande först.");
      return;
    }
    ...
    setTemplateStatus("Informationsmallen är sparad.");
  }
  async function save(mode: "schedule" | "now", quick?: string) {
    if (!draft.title.trim() || !draft.body.trim()) return;
    setSaving(true);
    let start = new Date(draft.start);
    let end = draft.end ? new Date(draft.end) : null;
    if (mode === "now") {
      start = new Date();
      if (quick === "30") end = new Date(Date.now() + 1800000);
      if (quick === "60") end = new Date(Date.now() + 3600000);
      if (quick === "day") {
        end = new Date();
    ...
    if (editing) await updateDoc(doc(db, "messages", id), payload);
    else
    ...
    <>
      <header className="page-header">
        <div>
          ...
          <h1>{editing ? "Redigera information" : "Ny information"}</h1>
        </div>
      </header>
      <div className="editor-grid">
        ...
        >
          ...
          <div className="two-col">
            <label>
              ...
              <select
                value={draft.type}
                onChange={(e) =>
                  setDraft({ ...draft, type: e.target.value as MessageType })
                }
              >
          ...
          <fieldset className="quick-publish">
            <legend>Publicera nu</legend>
            <button type="button" onClick={() => save("now", "30")}>
              30 minuter
            </button>
            <button type="button" onClick={() => save("now", "60")}>
              1 timme
            </button>
            <button type="button" onClick={() => save("now", "day")}>
              Resten av dagen
            </button>
            <button type="button" onClick={() => save("now")}>
              {draft.end ? "Egen sluttid" : "Tills vidare"}
            </button>
          </fieldset>
        </form>
        <aside className={`preview-card ${preview ? "preview-open" : ""}`}>
...
async function removePresentation(id: string) {
  ...
  batch.delete(doc(db, "presentations", id));
  await batch.commit();
}
...
function OpeningHoursEditor({
  ...
  function update(day: WeekdayKey, changes: Partial<OpeningHours[WeekdayKey]>) {
    onChange({ ...value, [day]: { ...value[day], ...changes } });
  }
  return (
    <section className="opening-hours-editor">
      <div>
        <h2>Öppettider</h2>
        <p>
          Disken är stängd som standard. Aktivera de dagar och tider då den är
          öppen. Lunch är valfri.
        </p>
      </div>
      <div className="hours-table">
        <div className="hours-heading" aria-hidden="true">
          <span>Dag</span>
          <span>Öppet</span>
          <span>Från</span>
          <span>Till</span>
          <span>Lunch från</span>
          <span>Lunch till</span>
        </div>
        {weekdays.map((day) => {
          const item = value[day];
          return (
            <div
              className={`hours-day-row ${item.enabled ? "enabled" : ""}`}
              key={day}
            >
              <strong>{weekdayLabels[day]}</strong>
              <label className="hours-enabled">
                <input
                  type="checkbox"
                  checked={item.enabled}
                  onChange={(e) => update(day, { enabled: e.target.checked })}
                />
                <span>{item.enabled ? "Ja" : "Nej"}</span>
              </label>
              <label>
                <span>Från</span>
                <input
      ...
      for (let start = 0; start < chunkCount; start += 10) {
        ...
        for (let i = start; i < Math.min(start + 10, chunkCount); i++) {
          ...
          batch.set(
            ...
            { index: i, data: Bytes.fromUint8Array(chunk) },
          );
        }
        setUploadStatus(
      ...
      <div className="settings-grid">
        <form className="editor-card" onSubmit={submit}>
          ...
          <div className="two-col">
            <label>
              ...
              <input
                ...
                onChange={(e) =>
                  setForm({ ...form, slideDuration: +e.target.value })
                }
              />
            </label>
            <label>
              Övergång (millisekunder)
              <input
                type="number"
                min="0"
                max="5000"
                ...
                onChange={(e) =>
                  setForm({ ...form, transitionDuration: +e.target.value })
                }
              />
            </label>
            <label>
              Rotationstid (sekunder)
              <input
                type="number"
                min="3"
                max="300"
                value={form.messageRotationTime}
                onChange={(e) =>
                  setForm({ ...form, messageRotationTime: +e.target.value })
                }
              />
            </label>
            <label>
          ...
          <details>
            ...
            <label>
              Slides{" "}
              <span className="label-help">En publik bild-URL per rad</span>
              <textarea
                rows={5}
                value={form.slides.join("\n")}
                onChange={(e) =>
                  setForm({
                    ...form,
                    presentationId: "",
          ...
          <button className="button primary">Spara inställningar</button>
          {saved && (
        ...
        <aside className="preview-card preview-open">
          <div className="preview-label">
            <span>Förhandsvisning</span>
            <span>16:9</span>
          </div>
