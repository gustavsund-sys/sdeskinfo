...
export function DisplayCanvas({settings,messages,preview=false}:{settings:DisplaySettings;messages:ServiceMessage[];preview?:boolean}){
 ...
 const active=useMemo(()=>messages.filter(m=>activeAt(m,now)).sort((a,b)=>({urgent:0,important:1,normal:2}[a.priority]-{urgent:0,important:1,normal:2}[b.priority])),[messages,now]);
 useEffect(()=>{if(!active.length)return;const id=setInterval(()=>setMessageIndex(i=>(i+1)%active.length),Math.max(3,settings.messageRotationTime)*1000);return()=>clearInterval(id)},[active.length,settings.messageRotationTime]);
 ...
 return <main className={`display-canvas ${message?'has-message':''} ${urgent?'is-urgent':''} ${preview?'is-preview':''}`}>
  ...
  <OpeningHoursPanel hours={settings.openingHours} now={now} width={message?`${width}%`:undefined}/>
 </main>
}
